...
await conn.sendButtonImg(m.chat, cita, `Nihh ${command} nya @${m.sender.split('@')[0]}`, wm, `🍆 LANJUT 🍆`, `${usedPrefix}${command}`, m, {mentions: [m.sender], jpegThumbnail: await(await fetch(cita)).buffer()})
}
if (command == 'akiyama') {
...
await conn.sendButtonImg(m.chat, json.url, `Nihh ${command} nya @${m.sender.split('@')[0]}`, wm, `🍆 LANJUT 🍆`, `${usedPrefix}${command}`, m, {mentions: [m.sender], jpegThumbnail: await(await fetch(json.url)).buffer()})
}
if (command == 'yatogami') {
